...
@Component({
  selector: 'app-list-alcohol-by-type',
  templateUrl: './list-alcohol-by-type.component.html',
  styleUrls: ['./list-alcohol-by-type.component.css']
})
export class ListAlcoholByTypeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getAllAcoholByOriginIdOfProject();
    this.getAllAcoholByOriginIdOfProjectNoPagination();
  }
  ...
  getAllAcoholByOriginIdOfProject() {
    this.productService.getAllAcoholByOriginId(this.typeId, this.offset).subscribe((data) => {
      this.products = data;
    });
  }
  ...
  reloadComponent(typeId, typeName) {
    const currentUrl = `/origin/${typeId}/${typeName}`;
